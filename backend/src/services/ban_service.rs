...
pub async fn list_bans(db: &Database, query: &ListQuery) -> anyhow::Result<crate::routes::PaginatedResponse<BanItem>> {
    ...
    let data_sql = format!("SELECT {BAN_FIELDS} FROM ban_records {where_clause} ORDER BY created_at DESC LIMIT ${param_idx} OFFSET ${}", param_idx + 1);

    let mut count_query = sqlx::query_scalar::<_, i64>(&count_sql);
    let mut data_query = sqlx::query_as::<_, BanRow>(&data_sql);
