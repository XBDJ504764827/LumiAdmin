...
| **白名单管理** | 白名单审核大厅（待审核/已通过/未通过三个标签页）、全球封禁记录检测（KZTimerGlobal API）、手动添加白名单、Steam 名称刷新、全球封禁玩家审核强制填写理由、低风险自动通过开关（低风险玩家申请满 3 小时无人审核自动通过）、**两步验证（Steam 身份 + QQ 私聊验证码绑定）**、**管理员私聊玩家**、QQ 绑定设置 |
...
| 页面 | 说明 |
|------|------|
| 白名单申请 | 玩家自助提交白名单申请：Steam 身份验证（登录或手动）→ 添加机器人 QQ 私聊发送验证码完成绑定 → 填写申请理由 |
| 白名单公示 | 已通过白名单公开展示 |
| 封禁公示 | 公开播封记录查看 |
...
### 白名单两步验证（Steam + QQ 私聊绑定）

为避免玩家填写错误联系方式，公开白名单申请改为两步验证：

1. **Steam 验证**：通过 Steam 登录（OpenID）或手动填写 Steam 标识符确认账号。
2. **QQ 私聊绑定**：网站在第一步后自动生成一次性验证码（`WL-XXXXXX`，默认 **5 分钟**有效，
   单个 Steam 仅保留一条活跃码，成功后立即作废，过期需重新生成）。玩家添加机器人 QQ
   好友后 **私聊直接发送验证码**（无需加群、无需 @），LumiBot 调用
   `POST /api/integration/qq/bind/verify` 完成 `steamid64 ↔ QQ(openid)` 绑定，并在私聊
   **被动回复**结果。
3. **填写理由**：绑定成功后回到网站填写申请理由并提交，等待管理员审核。

约束与说明：

- 绑定永久有效；`revoked` / `expired` 重新申请复用绑定，无需重新私聊。
- 1 个 QQ（openid）默认最多绑定 **5** 个 Steam；换绑需管理员先解绑。
- 申请的联系方式由绑定的 QQ openid 自动写入（`contact` = openid），不再手填。
- QQ 官方 Bot 只能拿到用户 C2C 场景的 openid（非真实 QQ 号）。
- 机器人名称 / 机器人 QQ 号 / 绑定上限 / 验证码有效期均可在
  白名单管理页的「QQ 绑定设置」中修改（`whitelist_qq_config`，仅 `developer` 可改）。
- 关闭该功能（`enabled = false`）时不强制绑定，兼容历史部署。

> 早期版本使用「QQ 群内 @机器人」绑定与「群内 @玩家」，因 QQ 平台
> **群主动消息无权限**（40034105）限制，已改为私聊方案。

管理侧接口与管理员私聊：

- `GET /api/whitelist/qq-binding/:steamid64`：查询绑定详情、同 QQ 关联账号与聊天记录。
- `DELETE /api/whitelist/qq-binding/:steamid64`：管理员解绑（换绑前必须先解绑）。
- `GET /api/whitelist/qq-chat/:steamid64`：读取与玩家的私聊记录。
- `POST /api/whitelist/qq-chat/:steamid64`：管理员私聊玩家（≤500 字）。权限
  `developer/admin`，成功/失败均写入 `qq_chat_messages`，后端通过
  `POST {LUMI_BOT_API_URL}/api/v1/messages/chat` 调用 LumiBot 发送。
- `GET/PUT /api/whitelist/qq-config`：读取/更新 QQ 绑定设置（仅 `developer`）。
- 玩家私聊消息由 LumiBot 回传 `POST /api/integration/qq/chat/inbound`，按 openid 归属到 Steam。

LumiBot 侧需配置 `LUMIADMIN_URL` 与 `LUMIADMIN_QQ_TOKEN`（与
`QQ_INTEGRATION_TOKEN` 一致）以启用绑定与私聊回传；管理员私聊复用
`LUMI_BOT_API_URL/KEY`（`X-API-Key`）。
...
| GET | `/api/whitelist/qq-config` | 读取 QQ 绑定设置 |
| PUT | `/api/whitelist/qq-config` | 更新 QQ 绑定设置 |
| GET | `/api/whitelist/qq-binding/:steamid64` | 查询 QQ 绑定详情 |
| DELETE | `/api/whitelist/qq-binding/:steamid64` | 解绑 Steam↔QQ |
| GET | `/api/whitelist/qq-chat/:steamid64` | 读取 QQ 私聊记录 |
| POST | `/api/whitelist/qq-chat/:steamid64` | 管理员私聊玩家 |
...
| POST | `/api/public/whitelist/qq-code` | 生成 QQ 绑定验证码 |
...
| POST | `/api/integration/qq/bind/verify` | QQ 验证码绑定校验（LumiBot 调用） |
| POST | `/api/integration/qq/chat/inbound` | 玩家私聊消息回传（LumiBot 调用） |
