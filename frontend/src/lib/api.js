...
export const api = {
  ...
  mapSyncOverview: (token) => request('/api/map-sync/config', { headers: withAuth(token) }),
  updateMapSyncConfig: (token, body) => request('/api/map-sync/config', { method: 'PUT', headers: withAuth(token), body: JSON.stringify(body) }),
  createMapSyncAgent: (token, body) => request('/api/map-sync/agents', { method: 'POST', headers: withAuth(token), body: JSON.stringify(body) }),
  deleteMapSyncAgent: (token, id) => request(`/api/map-sync/agents/${id}`, { method: 'DELETE', headers: withAuth(token) }),
  resetMapSyncAgentToken: (token, id) => request(`/api/map-sync/agents/${id}/reset-token`, { method: 'POST', headers: withAuth(token), body: JSON.stringify({}) }),
  checkMaps: (token) => request('/api/map-sync/check', { method: 'POST', headers: withAuth(token), body: JSON.stringify({}) }),
  syncSingleMap: (token, mapName) => request(`/api/map-sync/maps/${encodeURIComponent(mapName)}/sync`, { method: 'POST', headers: withAuth(token), body: JSON.stringify({}) }),
  ...
  externalBanApiSyncs: (token, params = {}) => request(`/api/external-ban-api/syncs${buildQueryString(params)}`, { headers: withAuth(token) }),
  banSyncStatus: (token, banId) => request(`/api/external-ban-api/bans/${banId}/sync-status`, { headers: withAuth(token) }),
  retryFailedExternalBanSyncs: (token) => request('/api/external-ban-api/syncs/retry-failed', { method: 'POST', headers: withAuth(token), body: JSON.stringify({}) }),
  ...
  uploadAdminReportFiles: (token, reportId, formData) => request(`/api/player-reports/${reportId}/files`, { method: 'POST', headers: withAuth(token), body: formData }),
  listAdminAppealFiles: (token, appealId) => request(`/api/ban-appeals/${appealId}/files`, { headers: withAuth(token) }),
  uploadAdminAppealFiles: (token, appealId, formData) => request(`/api/ban-appeals/${appealId}/files`, { method: 'POST', headers: withAuth(token), body: formData }),
