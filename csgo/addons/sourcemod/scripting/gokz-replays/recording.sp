...
bool ForceSaveRecordingForReview(int client, int course, float time, int teleportsUsed)
{
	...
	// The normal replay path has already scheduled its post-run save.
	if (postRunRecording[client] || runningRunBreatherTimer[client] != INVALID_HANDLE)
	{
		return true;
	}

	if (recordedRunData[client] == null || recordedRunData[client].Length == 0)
	{
		return false;
	}

	// PB/server-record checks may have disabled saving. Re-enable it as a
	// temporary replay so recordguard can copy it before normal cleanup.
	replaySaveState[client] = ReplaySave_Temp;
	GOKZ_OnTimerEnd_Recording(client, course, time, teleportsUsed);
	return postRunRecording[client] || runningRunBreatherTimer[client] != INVALID_HANDLE;
}
