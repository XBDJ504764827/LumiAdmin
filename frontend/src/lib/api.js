...
export const api = {
  ...
  mapFeedback: (token, params = {}) => request(`/api/map-feedback${buildQueryString(params)}`, { headers: withAuth(token) }),
  reviewMapFeedback: (token, id, body = {}) => request(`/api/map-feedback/${id}/review`, { method: 'POST', headers: withAuth(token), body: JSON.stringify(body) }),
