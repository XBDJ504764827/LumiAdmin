...
{
	name = "CNGOKZ Global", 
	author = "DanZay, CNGOKZ", 
	description = "Provides GOKZ global records with LumiAdmin abnormal record review", 
...
int gI_MapFileSize;
int gI_MapTier;
bool gB_LegacyGlobalSurfaceDeferred;

ConVar gCV_gokz_settings_enforcer;
ConVar gCV_gokz_warn_for_non_global_map;
ConVar gCV_EnforcedCVar[ENFORCEDCVAR_COUNT];

#include "cngokz-global/legacy_disable.sp"
#include "cngokz-global/api.sp"
#include "cngokz-global/ban_player.sp"
#include "cngokz-global/commands.sp"
...
{
	if (IsLegacyGlobalSurfaceOccupied())
	{
		gB_LegacyGlobalSurfaceDeferred = true;
	}
	else
	{
		CreateNatives();
		RegPluginLibrary("cngokz-global");
...
public void OnPluginStart()
{
	...
	{
		SetFailState("cngokz-global currently only supports 128 tickrate servers.");
	}
	if (FindCommandLineParam("-insecure") || FindCommandLineParam("-tools"))
	{
		SetFailState("cngokz-global currently only supports VAC-secured servers.");
	}
