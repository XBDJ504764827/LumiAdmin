...
void CreateNatives()
{
	...
	CreateNative("CNGOKZ_RP_ForceSaveRun", Native_CNGOKZ_RP_ForceSaveRun);
}
...
public int Native_CNGOKZ_RP_ForceSaveRun(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	int course = GetNativeCell(2);
	float time = view_as<float>(GetNativeCell(3));
	int teleportsUsed = GetNativeCell(4);
	return ForceSaveRecordingForReview(client, course, time, teleportsUsed);
}
